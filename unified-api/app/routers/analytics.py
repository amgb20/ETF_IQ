...
async def get_risk_metrics(
    ...
    positions = (
        await db.execute(
            select(Position)
            .where(Position.portfolio_id == portfolio_id, Position.is_active == True)
        )
    ).scalars().all()
    ...
    for eid in etf_ids:
        ...
        prices_result = await db.execute(
            select(Price)
            .where(and_(Price.etf_id == eid, Price.date >= one_year_ago))
            .order_by(Price.date)
        )
        rows = prices_result.scalars().all()

        if len(rows) < 2:
            metrics.append(ETFRiskMetric(
                etf_id=str(eid), isin=etf.isin, ticker_yf=etf.ticker_yf,
                name=etf.name, annualized_return=None,
                annualized_volatility=None, max_drawdown=None, sharpe_ratio=None,
            ))
            continue
    ...
    if len(isins) >= 2:
        ...
        shared_dates = sorted(
            set.intersection(*(set(a.keys()) for a in aligned.values()))
        )

        if len(shared_dates) >= 10:
            matrix_data = np.array([
                [aligned[isin_key][d] for d in shared_dates]
                for isin_key in isins
            ])
