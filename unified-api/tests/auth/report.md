...
| **Command** | `python -m pytest tests/auth/ --cov=app/auth --cov-report=term-missing -q -W ignore` |
| **Status** | PASS |
| **Passed** | 166 |
| **Failed** | 0 |
| **Skipped** | 0 |
...
| test_auth0.py::test_get_jwks_keeps_stale_cache_on_http_error | PASS | NEW |
| test_auth0.py::test_get_jwks_raises_on_error_with_empty_cache | PASS | NEW |
| test_auth0.py::test_get_jwks_keeps_stale_cache_on_network_error | PASS | NEW |
...
| test_router.py::test_refresh_returns_401_when_token_blocklisted | PASS | NEW |
...
- 4 new tests added: refresh blocklist check (1), JWKS error handling (3) — all marked NEW.
- `auth0_management.py` has **0% coverage** — scaffold module, not wired into any router yet.
- All other modules at 95%+ coverage; `audit.py`, `dependencies.py`, `jwt.py`, `jwt_utils.py` at 100%.
