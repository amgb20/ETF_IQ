...
def _common_patches(mock_llm, web_result: str = "Gold: $2,650/oz", rag_results=None):
    ...
    return [
        ...
        patch("app.agents.chat_agent.execute_web_search", return_value=web_result),
...
async def test_gold_price_triggers_web_search():
    ...
    mock_llm = _make_mock_llm([
        ...
        [_text_chunk("Based on live data, gold is currently trading at "),
         _text_chunk("$2,650 per troy ounce.")],
    ])
    ...
    assert "2,650" in full_text or "gold" in full_text.lower(), (
        f"Expected gold price in response, got: {full_text!r}"
    )
...
async def test_portfolio_value_answered_from_context():
    ...
    assert not tool_events, (
        f"Expected no tool events for a portfolio value question, got: {tool_events}"
    )
...
async def test_general_question_no_tool():
    ...
    mock_llm = _make_mock_llm([
        [_text_chunk(
            "An ETF (Exchange-Traded Fund) is a basket of securities "
...
async def test_hybrid_question_fires_both_tools():
        ...
        [AIMessageChunk(
            ...
            tool_calls=[
                {"name": "web_search",
                 ...
                 "id": "tc_web", "type": "tool_call"},
                {"name": "search_portfolio_knowledge",
                 "args": {"query": "gold position analysis"},
                 "id": "tc_rag", "type": "tool_call"},
            ],
        )],
        # Turn 2 — synthesis
        [_text_chunk(
            "Gold is at $2,650/oz. "
            "Past analysis rated your gold allocation as overweight."
        )],
    ])
    ...
    assert "web_search" in tool_names, (
        f"Expected web_search tool event, got tool names: {tool_names}"
    )
    assert "rag_search" in tool_names, (
        f"Expected rag_search tool event, got tool names: {tool_names}"
    )
    assert events[-1]["type"] == "done"
